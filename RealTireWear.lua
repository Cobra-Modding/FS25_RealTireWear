...
-- Keep original wheel indices for savegames and network events. Physics-only
-- wheels (rollers, packers and support contacts) are not serviceable tires.
...
function TireWear:showInfo(superFunc, box)
    ...
    local axles = TireReplaceEvent.getServiceAxles(self)
    if #axles == 0 then return superFunc(self, box) end
    local function addAxleLines()
        for index, axle in ipairs(axles) do
            local wear, _, count = TireReplaceEvent.getTireData(self, axle.mode)
            if count > 0 then
                local label
                if #axles == 1 then
                    label = g_i18n:getText("rtw_display_title", TireWear.MOD_NAME)
                else
                    label = g_i18n:getText("rtw_display_title", TireWear.MOD_NAME) .. " " .. axle.title
                end
                local value = string.format("%d %%", math.floor(TireWear.clamp(wear, 0, 1) * 100 + 0.5))
                box:addLine(label, value)
            end
        end
    end
